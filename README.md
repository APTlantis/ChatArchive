...
> **Release scope:** v0.1.2 established the Windows-first Microsoft Store release using MSIX as the public Windows artifact. NSIS/MSI are not the public v0.1.2 release contract. macOS and Linux support stay pending VM verification, and Project Intelligence is deferred from the release surface. See [Release v0.1.2 Scope](docs/Release-v0.1.2-Scope.md).
...
- The old installer-state restoration check failed. It remains historical cautionary evidence; Store distribution is now the public Windows release path instead of NSIS/MSI.
...
1. Complete VM verification before claiming macOS or Linux readiness.
2. Add a privacy scrubber before broader sharing.
3. Add the next provider adapter behind the existing `ProviderImporter` boundary.
