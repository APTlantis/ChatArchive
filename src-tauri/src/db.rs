...
fn read_app_settings(app: &AppHandle) -> AppResult<AppSettings> {
    ...
    let text =
        fs::read_to_string(&path).map_err(|err| format!("Could not read settings: {err}"))?;
...
pub fn configured_library(app: &AppHandle) -> AppResult<Option<PathBuf>> {
    let library = read_app_settings(app)?.library_path.map(PathBuf::from);
    if let Some(path) = &library {
        allow_library_assets(app, path)?;
    }
    Ok(library)
}
...
fn allow_library_assets(app: &AppHandle, library: &Path) -> AppResult<()> {
    app.asset_protocol_scope()
        .allow_directory(library, true)
        .map_err(|err| format!("Could not allow library assets: {err}"))
}

pub fn ensure_library_layout(library: &Path) -> AppResult<()> {
    fs::create_dir_all(library.join("archives"))
        .map_err(|err| format!("Could not create archives folder: {err}"))?;
    fs::create_dir_all(library.join("exports"))
...
pub fn open_db(library: &Path) -> AppResult<Connection> {
    Connection::open(library.join("chatarchive.db"))
...
pub fn get_meta<T: for<'de> serde::Deserialize<'de>>(
    conn: &Connection,
    key: &str,
) -> AppResult<Option<T>> {
    ...
    raw.map(|text| {
        serde_json::from_str(&text).map_err(|err| format!("Could not parse app metadata: {err}"))
    })
    .transpose()
}
...
pub fn replace_archive(
    ...
    let tx = conn
        .transaction()
        .map_err(|err| format!("Could not begin import transaction: {err}"))?;
    tx.execute_batch(
        r#"
  ...
  .map_err(|err| format!("Could not write archive row: {err}"))?;
    ...
    let mut artifact_text: std::collections::BTreeMap<String, String> =
        std::collections::BTreeMap::new();
    for item in &artifacts.code {
        artifact_text
            .entry(item.base.conversation_id.clone())
            .or_default()
    ...
    for row in rows {
        ...
        artifacts.push(
            serde_json::from_str(&raw)
                .map_err(|err| format!("Could not parse code artifact JSON: {err}"))?,
        );
    }
    Ok(artifacts)
}
...
pub fn load_conversation(
    ...
    conn: &Connection,
    id: &str,
) -> AppResult<Option<ConversationFile>> {
    let path: Option<String> = conn
        .query_row(
            "SELECT json_path FROM conversations WHERE id = ?1",
            params![id],
            |row| row.get(0),
        )
        .optional()
    ...
    let text = fs::read_to_string(library.join(path))
        .map_err(|err| format!("Could not read conversation JSON: {err}"))?;
    serde_json::from_str(&text)
        .map(Some)
        .map_err(|err| format!("Could not parse conversation JSON: {err}"))
}
...
pub fn load_viewer_state(conn: &Connection) -> AppResult<ViewerState> {
    ...
    {
        let mut stmt = conn
            .prepare("SELECT conversation_id, created_at FROM favorite_conversations")
    ...
    {
        let mut stmt = conn
            .prepare("SELECT conversation_id, created_at FROM pinned_conversations")
            .map_err(|err| err.to_string())?;
        let rows = stmt
            .query_map([], |row| {
                Ok(ConversationBookmark {
    ...
    {
        let mut stmt = conn
            .prepare("SELECT conversation_id, read_at FROM read_conversations")
            .map_err(|err| err.to_string())?;
        let rows = stmt
            .query_map([], |row| {
    ...
    }
    {
        ...
        for row in rows {
            state
                .recently_viewed
                .push(row.map_err(|err| err.to_string())?);
        }
    }
    {
    ...
    {
        let mut stmt = conn
            .prepare("SELECT conversation_id, position FROM scroll_positions")
            .map_err(|err| err.to_string())?;
        let rows = stmt
            .query_map([], |row| {
...
pub fn replace_viewer_state(conn: &mut Connection, state: &ViewerState) -> AppResult<ViewerState> {
    let tx = conn
        .transaction()
        .map_err(|err| format!("Could not begin state transaction: {err}"))?;
    tx.execute_batch(
  ...
  .map_err(|err| format!("Could not clear viewer state: {err}"))?;
    for item in state.favorites.values() {
        tx.execute(
            ...
            params![item.conversation_id, item.created_at],
        )
        .map_err(|err| err.to_string())?;
    }
    ...
    for item in state.recently_viewed.iter().take(12) {
        tx.execute(
            "INSERT INTO recently_viewed(conversation_id, viewed_at) VALUES (?1, ?2)",
            params![item.conversation_id, item.viewed_at],
        )
        .map_err(|err| err.to_string())?;
    }
    ...
    .map_err(|err| err.to_string())?;
    tx.commit()
        .map_err(|err| format!("Could not commit viewer state: {err}"))?;
...
pub fn state_migrated(conn: &Connection) -> AppResult<bool> {
    let value: Option<String> = conn
        .query_row(
            "SELECT value FROM app_meta WHERE key = 'viewer_state_migrated'",
            [],
            |row| row.get(0),
        )
        .optional()
...
pub fn stored_conversation_summaries(conn: &Connection) -> AppResult<Vec<ConversationSummary>> {
    ...
    let rows = stmt
        .query_map([], |row| {
            ...
            let value: ConversationSummary = serde_json::from_str(&raw).map_err(|err| {
