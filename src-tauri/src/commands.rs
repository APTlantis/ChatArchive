...
fn library_or_error(app: &AppHandle) -> AppResult<PathBuf> {
  let library = db::configured_library(app)?.ok_or("No ChatArchive library folder has been selected")?;
...
#[tauri::command]
pub fn select_library_folder(app: AppHandle, library_path: String) -> Result<LibraryStatus, String> {
  db::set_configured_library(&app, Path::new(&library_path))?;
  get_library_status(app)
}

#[tauri::command]
pub fn import_openai_export(app: AppHandle, source_path: String, library_path: Option<String>) -> Result<ImportSummary, String> {
...
#[tauri::command]
pub fn get_conversation(app: AppHandle, conversation_id: String) -> Result<ConversationFile, String> {
  let (library, conn) = open_library_db(&app)?;
  db::load_conversation(&library, &conn, &conversation_id)?.ok_or(format!("Conversation not found: {conversation_id}"))
}
...
#[tauri::command]
pub fn search_conversations(app: AppHandle, filters: SearchFilters) -> Result<Vec<ConversationSummary>, String> {
  let (_, conn) = open_library_db(&app)?;
  ...
  let terms = query.split_whitespace().filter(|part| !part.contains(':')).map(ToString::to_string).collect::<Vec<_>>();
...
#[tauri::command]
pub fn update_viewer_state(app: AppHandle, viewer_state: ViewerState) -> Result<ViewerState, String> {
...
pub fn toggle_favorite(app: AppHandle, conversation_id: String) -> Result<ViewerState, String> {
  ...
  let exists: Option<i64> = conn
    .query_row("SELECT created_at FROM favorite_conversations WHERE conversation_id = ?1", params![conversation_id], |row| row.get(0))
    .optional()
    .map_err(|err| err.to_string())?;
  if exists.is_some() {
    conn.execute("DELETE FROM favorite_conversations WHERE conversation_id = ?1", params![conversation_id]).map_err(|err| err.to_string())?;
  } else {
    conn.execute("INSERT INTO favorite_conversations(conversation_id, created_at) VALUES (?1, ?2)", params![conversation_id, db::now_ms()])
      .map_err(|err| err.to_string())?;
  }
...
pub fn toggle_pin(app: AppHandle, conversation_id: String) -> Result<ViewerState, String> {
  let (_, conn) = open_library_db(&app)?;
  let exists: Option<i64> = conn
    .query_row("SELECT created_at FROM pinned_conversations WHERE conversation_id = ?1", params![conversation_id], |row| row.get(0))
    .optional()
    .map_err(|err| err.to_string())?;
  if exists.is_some() {
    conn.execute("DELETE FROM pinned_conversations WHERE conversation_id = ?1", params![conversation_id]).map_err(|err| err.to_string())?;
  } else {
    conn.execute("INSERT INTO pinned_conversations(conversation_id, created_at) VALUES (?1, ?2)", params![conversation_id, db::now_ms()])
      .map_err(|err| err.to_string())?;
  }
  db::load_viewer_state(&conn)
}
...
#[tauri::command]
pub fn mark_read(app: AppHandle, conversation_id: String, read: bool) -> Result<ViewerState, String> {
  let (_, conn) = open_library_db(&app)?;
  if read {
    conn.execute(
    ...
    conn.execute("DELETE FROM read_conversations WHERE conversation_id = ?1", params![conversation_id]).map_err(|err| err.to_string())?;
  }
  db::load_viewer_state(&conn)
}

#[tauri::command]
pub fn save_message_bookmark(app: AppHandle, bookmark: MessageBookmark, bookmarked: bool) -> Result<ViewerState, String> {
  let (_, conn) = open_library_db(&app)?;
  if bookmarked {
    conn.execute(
...
#[tauri::command]
pub fn save_scroll_position(app: AppHandle, conversation_id: String, position: f64) -> Result<ViewerState, String> {
  let (_, conn) = open_library_db(&app)?;
  conn.execute(
...
#[tauri::command]
pub fn export_conversation_markdown(app: AppHandle, conversation_id: String, markdown: String) -> Result<String, String> {
  let (library, conn) = open_library_db(&app)?;
  let conversation = db::load_conversation(&library, &conn, &conversation_id)?.ok_or("Conversation not found")?;
  let archive_path = db::active_archive_path(&conn)?.ok_or("No active archive folder found")?;
  let exports = archive_path.join("exports");
  fs::create_dir_all(&exports).map_err(|err| format!("Could not create exports folder: {err}"))?;
  let file = exports.join(format!("{}-{}.md", conversation.summary.slug, Utc::now().format("%Y%m%d%H%M%S")));
